...
function formatData(data, passages, filterString) {
  ...
  for (var index=0; index<data.results.length; index++) {
    ...
    var initialTextLen = (dataItem.text.length > 800)? 800 : Math.ceil(dataItem.text.length / 2);
    // make sure we end between words
    initialTextLen = dataItem.text.lastIndexOf(' ', initialTextLen);

    // console.log(dataItem.result_metadata)
    var newResult = {
      id: dataItem.id,
      sessionToken: data.sessionToken,
      title: (!dataItem.extracted_metadata.filename)? dataItem.extracted_metadata.title: dataItem.extracted_metadata.filename,
      // text: dataItem.text,
      textBlurb: dataItem.text.substr(0, initialTextLen),
      textFull: dataItem.text,
      url: dataItem.metadata.source.url,
      score: (!dataItem.result_metadata.confidence)? dataItem.result_metadata.score: dataItem.result_metadata.confidence,
